...
$new_files = ['404.php','css/login.css','js/login.js','css/main.css','js/main.js'];
...
register_nav_menus(array('menu_main' 	=> __('Menú principal', 'bilnea')));
register_nav_menus(array('menu_footer'	=> __('Barra inferior', 'bilnea')));
register_nav_menus(array('menu_top' 	=> __('Barra superior', 'bilnea')));
register_nav_menus(array('menu_widget' 	=> __('Menú widget', 'bilnea')));
...
function b_f_sidebar() {
	register_sidebar(
		array(
			'id' => 'sidebar',
			'name' => __('Barra lateral', 'bilnea'),
			'description' => __('Barra lateral del sitio', 'bilnea'),
	...
	register_sidebar(
		array(
			'id' => 'sidebar_page',
			'name' => __('Barra lateral página', 'bilnea'),
			'description' => __('Barra lateral de página', 'bilnea'),
			'before_widget' => '<div id="%1$s" class="widget %2$s">',
			'after_widget' => '</div>',
			'before_title' => '<h4 class="side-title">',
			'after_title' => '</h4>',
	...
	register_sidebar(
		array(
			'id' => 'sidebar_blog',
			'name' => __('Barra lateral blog', 'bilnea'),
			'description' => __('Barra lateral del blog', 'bilnea'),
			'before_widget' => '<div id="%1$s" class="widget %2$s">',
			'after_widget' => '</div>',
			'before_title' => '<h4 class="side-title">',
			'after_title' => '</h4>',
	...
	register_sidebar(
		array(
			'id' => 'sidebar_alter1',
			'name' => __('Barra lateral alternativa #1', 'bilnea'),
			'description' => __('Barra lateral alternativa', 'bilnea'),
			'before_widget' => '<div id="%1$s" class="widget %2$s">',
			'after_widget' => '</div>',
			'before_title' => '<h4 class="side-title">',
			'after_title' => '</h4>',
	...
	register_sidebar(
		array(
			'id' => 'sidebar_alter2',
			'name' => __('Barra lateral alternativa #2', 'bilnea'),
			'description' => __('Barra lateral alternativa', 'bilnea'),
			'before_widget' => '<div id="%1$s" class="widget %2$s">',
			'after_widget' => '</div>',
			'before_title' => '<h4 class="side-title">',
	...
	for ($i=0; $i < b_f_option('b_opt_footer-menu'); $i++) { 
		switch ($i) {
			case 0:
				$txt = __('Primera', 'bilnea');
				break;
			case 1:
				$txt = __('Segunda', 'bilnea');
				break;
			case 2:
				$txt = __('Tercera', 'bilnea');
				break;
			case 3:
				$txt = __('Cuarta', 'bilnea');
				break;
			case 4:
				$txt = __('Quinta', 'bilnea');
				break;
		}
		register_sidebar(
			array(
				'id' => 'footer_'.($i+1),
				'name' => 'Footer #'.($i+1),
				'description' => $txt.__(' columna del pie de página', 'bilnea'),
...
function b_f_load()  {
	...
	if (b_f_option('b_opt_lightbox') == 1) {
		wp_enqueue_script('lightbox', get_template_directory_uri().'/js/magnific-popup.js', array('jquery'), $version, true);
	...
	if (file_exists(get_stylesheet_directory_uri().'/js/'.$post->post_type.'-'.$post->ID.'.js')) {
...
function b_f_login() {
	wp_enqueue_style('custom-login', get_stylesheet_directory_uri() .'/js/login.css');
	wp_enqueue_script('custom-login', get_stylesheet_directory_uri() .'/css/login.js');
}
...
function add_check(){
	...
	if (!is_user_logged_in()) {
		$out .= '<div id="b_sdiv" style="clear:both;"><input type="checkbox" id="b_schk" name="b_schk" />';
		$out .= '<p>Por favor, confirma que eres humano antes de comentar.</p></div>';
		$out .= '<noscript>Debes tener activo javascript para poder comentar</noscript>';
		$out .= '<input type="hidden" id="b_chk_email" name="b_chk_email" value="" />';
    }
    echo $out;
}

add_action('comment_form','add_check',1);
...
if (b_f_option('b_opt_create-legal-page') == 1) {
	create_pages_fly('legal');
} else {
	if (function_exists('icl_object_id')) {
		$lng = icl_get_languages('skip_missing=0&orderby=code');
		if (!empty($lng)) {
			foreach ($lng as $l) {
				wp_delete_post($options['b_opt_legal-id-'.$l['language_code']], true);
				$options = get_option('bilnea_settings');
				$options['b_opt_legal-id-'.$l['language_code']] = '';
				$options['b_opt_legal-url-'.$l['language_code']] = '';
				update_option('bilnea_settings', $options);
			}
		}
	} else {
		wp_delete_post($options['b_opt_legal-id-es'], true);
		$options = get_option('bilnea_settings');
		$options['b_opt_legal-id-es'] = '';
		$options['b_opt_legal-url-es'] = '';
		update_option('bilnea_settings', $options);
	}
}

if (b_f_option('b_opt_create-privacy-page') == 1) {
	create_pages_fly('privacy');
} else {
	if (function_exists('icl_object_id')) {
		$lng = icl_get_languages('skip_missing=0&orderby=code');
		if (!empty($lng)) {
...
function create_pages_fly($pgn) {
	...
	if (function_exists('icl_object_id')) {
		...
		if (!empty($lng)) {
			...
			$main_id = '';
			foreach ($lng as $l) {
				if(!file_exists(get_template_directory().'/inc/'.$l['language_code'].'/'.$pgn.'.php')) {
					include_once(get_template_directory().'/inc/es/'.$pgn.'.php');
				} else {
					include_once(get_template_directory().'/inc/'.$l['language_code'].'/'.$pgn.'.php');
				}
				if (b_f_option('b_opt_'.$pgn.'-id-'.$l['language_code']) == '') {
					$page = array(
						'post_title'    => b_f_default()['b_opt_name_'.$pgn.'-'.$l['language_code']],
						'post_content'  => $txt,
						'post_status'   => 'publish',
						'post_author'   => 1,
						'post_type'     => 'page',
						'post_name'     => b_f_default()['b_opt_'.$pgn.'-url-'.$l['language_code']],
						'post_parent'	=> 0,
						'page_template'	=> 'blank-page.php'
					);
					$new_id = wp_insert_post($page);
					$options = get_option('bilnea_settings');
					$options['b_opt_'.$pgn.'-id-'.$l['language_code']] = $new_id;
					if ($options['b_opt_'.$pgn.'-url-'.$l['language_code']] == '') {
						$options['b_opt_'.$pgn.'-url-'.$l['language_code']] = b_f_default()['b_opt_'.$pgn.'-url-'.$l['language_code']];
					}
					update_option('bilnea_settings', $options);
					if (b_f_option('b_opt_'.$pgn.'-id-'.$l['language_code']) == '' && $l['language_code'] == $idioma && get_page_by_title(b_f_default()['b_opt_name_'.$pgn.'-'.$l['language_code']]) == NULL) {
						$main_id = $new_id;
					...
					if (b_f_option($pgn.'_id_'.$l['language_code']) == '' && $l['language_code'] != $idioma && get_page_by_title(b_f_default()['b_opt_name_'.$pgn.'_'.$l['language_code']]) == NULL) {
					...
					$page = array(
						'ID'			=> b_f_option('b_opt_'.$pgn.'-id-'.$l['language_code']),
						'post_title'    => b_f_default()['b_opt_name_'.$pgn.'-'.$l['language_code']],
						'post_name'     => b_f_default()['b_opt_'.$pgn.'-url-'.$l['language_code']],
						'post_content'  => $txt
					);
