...
<div id="socket">
	<div <?php if (b_f_option('socket_width') == 2) { echo 'class="container"'; } ?>>
		...
		if (function_exists('icl_object_id')) {
			...
			if (b_f_option('b_opt_legal-url-'.$lang) != '') {
				array_push($sck, '<a href="'.$sitepress->language_url(ICL_LANGUAGE_CODE).b_f_option('b_opt_legal-url-'.$lang).'" target="_blank" title="'.__('Legal Advice', 'bilnea').'" rel="noindex">'.__('Legal Advice', 'bilnea').'</a>');
			}
			if (b_f_option('b_opt_privacy-url-'.$lang) != '') {
				array_push($sck, '<a href="'.$sitepress->language_url(ICL_LANGUAGE_CODE).b_f_option('b_opt_privacy-url-'.$lang).'" target="_blank" title="'.__('Privacy Policy', 'bilnea').'" rel="noindex">'.__('Privacy Policy', 'bilnea').'</a>');
			}
			if (b_f_option('b_opt_cookies-url-'.$lang) != '') {
				array_push($sck, '<a href="'.$sitepress->language_url(ICL_LANGUAGE_CODE).b_f_option('b_opt_cookies-url-'.$lang).'" target="_blank" title="'.__('Cookies Policy', 'bilnea').'" rel="noindex">'.__('Cookies Policy', 'bilnea').'</a>');
			}
		} else {
			$sck = [];
			if (b_f_option('socket_copy') == 1) {
		...
		if (md5(b_f_option('socket_no-development')) != 'e8f00e69e2bc444b3c291110d037eb7d') {
			array_push($sck, __('Web development by', 'bilnea').' <a href="http://bilnea.com" title="'.__('bilnea. Communication & Digital Marketing Agency', 'bilnea').'" target="_blank" rel="nofollow">bilnea</a>');
		}
