...
class bilnea_Blog extends Widget_Base {
	...
	protected function render() {
		...
		foreach (get_posts($args) as $post) {
			$temp = '<div class="'.(($settings['height'] == 'yes') ? 'auto-height ' : '').'elementor-column elementor-col-'.round(100/$settings['columns']['size']).'" data-id="'.get_post_type($post->ID).'-'.$post->ID.'">';
			$shortcodes = array(
				'{{b_title}}',
				'{{b_permalink}}',
				'{{b_date}}',
		...
		$total = count(get_posts($args)) - $settings['offset'];
