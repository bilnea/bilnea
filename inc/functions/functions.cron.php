...
if (!function_exists('b_f_weekly_cron')) {
	...
	function b_f_weekly_cron() {
		...
		if (!$var_fonts->error) {
			$var_file = @fopen(get_template_directory().'/inc/data/data.google.fonts.json', 'w');
			if ($var_file !== false) {
				ftruncate($var_file, 0);
				fwrite($var_file, b_f_get_file_content('https://www.googleapis.com/webfonts/v1/webfonts?key='.$b_g_google_api));
				fclose($var_file);
			}
