...
if (is_null(get_the_excerpt())) {
	$var_excerpt = wp_trim_words(get_the_content(), 30);
} else {
	$var_excerpt = get_the_excerpt();
}
...
$var_replace = array(get_the_title(), get_queried_object()->name, get_permalink(), $var_excerpt, get_the_content(), get_the_date(b_f_option('b_opt_blog-date-'.$b_g_language, true)), $var_categories, get_the_author_link(), $var_tags, $var_comments, '');
