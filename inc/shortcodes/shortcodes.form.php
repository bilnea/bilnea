	...
	function b_s_input($atts) {
		...
		switch (esc_attr($a['type'])) {
			...
			case 'checkbox':
				...
				$out .= '  <input data-name="'.$var_name.'" class="b_input_checkbox" type="checkbox" value="'.$var_placeholder.'" name="b_i_custom_checkbox-'.$var_key.'" id="checkbox-'.$var_random.'"><label for="checkbox-'.$var_random.'">'.$var_placeholder.'</label>'."\n";
				foreach ($var_options as $option) {
					$var_random = rand(0, 999);
					if (count(explode(':', $option)) > 1) {
						$option = explode(':', $option);
						$out .= '  <input data-name="'.$var_name.'" class="b_input_checkbox" type="checkbox" value="'.$option[0].'" name="b_i_custom_checkbox-'.$var_key.'" id="checkbox-'.$var_random.'"><label for="checkbox-'.$var_random.'">'.$option[1].'</label>'."\n";
					} else {
			...
			case 'send':
				...
				return '<div class="form-send '.$var_class.'" data-send="'.$var_placeholder.'" data-sending="'.$var_sending.'">'.$var_placeholder.'</div>';
				break;
