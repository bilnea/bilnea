...
<html <?php language_attributes(); ?>>
	...
	<body <?php body_class(); $anc = (b_f_option('b_opt_anticopy') == 1) ? ' onkeypress="return anticopia(event);" onkeydown="return anticopia(event);" oncontextmenu="return false;"' : ''; echo $anc; ?>>
		...
		?>
		<div id="cookie_warning" style="padding: 10px 40px 10px 10px; text-align: center; left: 0; right: 0; z-index: 100000; <?php echo $pab; ?>">
			<div class="container">
				<?php
				$cookies_url = '/';
				if (function_exists('icl_object_id')) {
					...
					if (b_f_option('b_opt_cookies-url-_'.$lang) != '') {
						$cookies_url = $sitepress->language_url(ICL_LANGUAGE_CODE).b_f_option('b_opt_cookies-url-_'.$lang);
					}
				} else {
					if (b_f_option('b_opt_cookies-url-_es') != '') {
						$cookies_url = site_url().'/'.b_f_option('b_opt_cookies-url-_es');
					}
				}
				printf(__('We use our own and third-part <em>cookies</em> to analyse the use and measurement of our web in order to improve our services. If you go on surfing, we assume you acceept their use. <br />You can change your settings or find more information <a href="%1$s" target="_blank" title="%2$s cookies policy">here</a>.', 'bilnea'), $cookies_url, get_bloginfo('name'));
				?>
	...
	?>
		...
		<header id="mobile-header" class="<?= $stk ?> mobile-header-side">
			<div>
				<div id="mobile-menu-button">
					<button class="mobile-button">
						<span>toggle menu</span>
