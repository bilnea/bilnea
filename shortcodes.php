...
function b_f_button($atts, $content = null) {
	...
	if (esc_attr($a['color']) != null) { $col = ' style="background-color: '.esc_attr($a['color']).';"'; }
	if (esc_attr($a['url']) != null) { $url = ' href="'.str_replace('b_root', preg_replace('(^https?://)', '', get_site_url()), esc_url(esc_attr($a['url']))).'"'; }
...
function form($atts, $content = null) {
	...
	$txt .= '</form>';
	$txt .= '<div id="form-send" data-send="'.__('Send', 'bilnea').'" data-sending="'.__('Sending', 'bilnea').'">'.__('Send', 'bilnea').'</div>';
...
add_shortcode('b_rposts', 'recientes');

function b_f_select_sidebar() {
	...
	$out  = '<aside id="sidebar" class="barra-lateral sidebar-right">';
...
function b_f_blog($atts) {
	...
	$o = array(
        'post_type' => 'post',
        'posts_per_page' => b_f_option('b_opt_blog-number'),
        'paged' => $pag,
